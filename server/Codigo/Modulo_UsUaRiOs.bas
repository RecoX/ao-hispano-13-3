                ...
                If InMapBounds(Map, X, Y) Then
                        ...
                        If Not toMap Then
                                ...
                                Call WriteCharacterCreate(sndIndex, .Char.Body, .Char.Head, .Char.heading, _
                                   ...
                                   UserName, NickColor, Privileges, 0, 0)
                        Else
...
Public Sub CheckUserLevel(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                ...
                If .Stats.ELV > Configuracion.NivelMaximo Then
                        .Stats.Exp = 0
                        .Stats.ELU = 0
                ...
                Do While .Stats.Exp >= .Stats.ELU
                        ...
                        ' @@ Nuevo sistema configurable desde Dat - Miqueas
                        .Stats.ELU = Get_ExpLvl(.Stats.ELV)
                    
                        ''Calculo subida de vida
                          
                        '// Nuevo sistema : Miqueas
                        '// MAB - Última actualización: 30/05/2015
                        '// Detalles: Se colocaron los valores por default de la 13.0, y se edito en 1 punto los mínimos de vida, parar lograr mejor promedio en todos los casos

                        Select Case .clase
    
                                Case eClass.Warrior

                                        Select Case .Stats.UserAtributos(Constitucion)
                                                ...
                                                Case Else
                                                        AumentoHP = RandomNumber(8, .Stats.UserAtributos(Constitucion) \ 2) + AdicionalHPGuerrero
                                ...
                                Case eClass.Worker
                                        ...
                                        Select Case .Stats.UserAtributos(Constitucion)
                                                ...
                                                Case Else
                                                        AumentoHP = RandomNumber(7, .Stats.UserAtributos(Constitucion) \ 2) + AdicionalHPGuerrero
                                ...
                                Case eClass.Cleric
                                        ...
                                        Select Case .Stats.UserAtributos(eAtributos.Constitucion)
                                                ...
                                                Case Else
                                                        AumentoHP = RandomNumber(6, .Stats.UserAtributos(Constitucion) \ 2)
                                ...
                                Case Else
                                        ...
                                        ' @@ Miqueas : Minimo Que tira de Vida 6.
                                        If AumentoHP <= 6 Then AumentoHP = 6
                ...
                If Pts > 0 Then
                        Call WriteLevelUp(UserIndex, 1, Pts)
...
Public Sub SendUserStatsTxt(ByVal sendIndex As Integer, ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                Call WriteConsoleMsg(sendIndex, "Estadísticas de: " & .Name, FontTypeNames.FONTTYPE_GUILDMSG)
                Call WriteConsoleMsg(sendIndex, "Nivel: " & .Stats.ELV & "  EXP: " & .Stats.Exp & "/" & .Stats.ELU, FontTypeNames.FONTTYPE_GUILDMSG)
                Call WriteConsoleMsg(sendIndex, "Salud: " & .Stats.MinHp & "/" & .Stats.MaxHP & "  Maná: " & .Stats.MinMAN & "/" & .Stats.MaxMAN & "  Energía: " & .Stats.MinSta & "/" & .Stats.MaxSta, FontTypeNames.FONTTYPE_GUILDMSG)
                Call WriteConsoleMsg(sendIndex, "Oro: " & UserList(UserIndex).Stats.GLD & " Boveda: " & UserList(UserIndex).Stats.Banco & " Posicion: " & UserList(UserIndex).Pos.X & "," & UserList(UserIndex).Pos.Y & " en mapa " & UserList(UserIndex).Pos.Map, FontTypeNames.FONTTYPE_GUILDMSG)
                Call WriteConsoleMsg(sendIndex, "Dados: " & .Stats.UserAtributos(eAtributos.Fuerza) & ", " & .Stats.UserAtributos(eAtributos.Agilidad) & ", " & .Stats.UserAtributos(eAtributos.Inteligencia) & ", " & .Stats.UserAtributos(eAtributos.Carisma) & ", " & .Stats.UserAtributos(eAtributos.Constitucion), FontTypeNames.FONTTYPE_GUILDMSG)
        ...
        Case eRaza.Humano
            Miraza = "Humano"
        End Select
        Call WriteConsoleMsg(sendIndex, "Clase: " & ListaClases(.clase) & " / Raza: " & Miraza & ".", FontTypeNames.FONTTYPE_GUILDMSG)
        
        GuildI = UserList(UserIndex).GuildIndex
    If GuildI > 0 Then
        Call WriteConsoleMsg(sendIndex, "Clan: " & modGuilds.GuildName(GuildI), FontTypeNames.FONTTYPE_GUILDMSG)
    ...
    'Items
    Call WriteConsoleMsg(sendIndex, "Inventario: Tiene " & .Invent.NroItems & " objetos.", FontTypeNames.FONTTYPE_GUILDMSG)
    For j = 1 To MAX_NORMAL_INVENTORY_SLOTS
        If .Invent.Object(j).objIndex > 0 Then
            Call WriteConsoleMsg(sendIndex, "Objeto " & j & " " & ObjData(.Invent.Object(j).objIndex).Name & " Cantidad:" & .Invent.Object(j).Amount, FontTypeNames.FONTTYPE_GUILDMSG)
        End If
    Next j
