...
Private Sub HandleLoginExistingChar(ByVal UserIndex As Integer)
        ...
        #If SeguridadAlkon Then
                ...
                'If Not MD5ok(buffer.ReadASCIIStringFixed(16)) Then
                 '       Call WriteErrorMsg(UserIndex, "El cliente está dañado, por favor descarguelo nuevamente desde www.hispanoao-oficial.com")
                'Else
                #End If
        
                If BANCheck(UserName) Then
                        Call WriteErrorMsg(UserIndex, "Se te ha prohibido la entrada a Hispano Online debido a tu mal comportamiento. Puedes consultar el reglamento y el sistema de soporte desde www.hispanoao-oficial.com")
                'ElseIf Not VersionOK(version) Then comente estas dos lineas,
                 '       Call WriteErrorMsg(UserIndex, "Esta versión del juego es obsoleta, la versión correcta es la " & ULTIMAVERSION & ". La misma se encuentra disponible en www.hispanoao-oficial.com")
                Else
...
Private Sub HandleLoginNewChar(ByVal UserIndex As Integer)
        ...
        'If Not VersionOK(version) Then
         '       Call WriteErrorMsg(UserIndex, "Esta versión del juego es obsoleta, la versión correcta es la " & ULTIMAVERSION & ". La misma se encuentra disponible en www.hispano-ao.com")
       ' Else
                        ...
                        If Not (.flags.AdminInvisible = 1) Then
                                ...
                                Else
                                        ...
                                        If .flags.Privilegios = PlayerType.User Then
                                                Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead(Chat, .Char.CharIndex, RGB(244, 244, 0)))
                                        Else
...
Private Sub HandleUserCommerceOffer(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                ...
                Else
                        ...
                        If ItemNewbie(objIndex) Or ItemShop(objIndex) Then
...
Private Sub HandleOnline(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If LenB(list) <> 0 Then
            ...
            Call WriteConsoleMsg(UserIndex, "Usuarios Online: " & list & ". (" & CStr(Count) & ")", FontTypeNames.FONTTYPE_CENTINELA)
        Else
            Call WriteConsoleMsg(UserIndex, "No hay usuarios online.", FontTypeNames.FONTTYPE_CENTINELA)
        End If
        ...
        With UserList(UserIndex)
                ...
                If (.flags.Privilegios And (PlayerType.Admin Or PlayerType.Dios Or PlayerType.SemiDios)) Then
                        If LenB(message) <> 0 Then
                                ...
                                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(.Name & "> " & message, FontTypeNames.FONTTYPE_DIOS))
...
Public Sub WriteCharacterCreate(ByVal UserIndex As Integer, _
        ...
        Call UserList(UserIndex).outgoingData.WriteASCIIStringFixed(PrepareMessageCharacterCreate(Body, Head, heading, CharIndex, X, Y, weapon, shield, FX, FXLoops, _
           helmet, Name, NickColor, Privileges, MinHp, MaxHP))
        Exit Sub
...
Private Sub HandleCanjer(ByVal UserIndex As Integer)
        ...
        '***************************************************
        'Author: ZheTa Adaptado para Hispano AO
        '***************************************************
        ...
        With UserList(UserIndex)
                ...
                If Not Mod_Cofres.GetCanje(UserIndex, .incomingData.ReadByte, tmpStr) Then ' @@ Miqueas : Nuevo Sistema
...
Private Sub HandleSetPointsShop(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                ...
                If (Not .flags.Privilegios) <> 0 And (.flags.Privilegios And (PlayerType.Admin Or PlayerType.Dios Or PlayerType.SemiDios Or PlayerType.RoleMaster)) <> 0 Then
                        ...
                        If uName <> "AMISHAR" And uName <> "MAB" And uName <> "AFRODITA" And uName <> "MOAS" Then
                                Call .incomingData.CopyBuffer(buffer)
                                Set buffer = Nothing
                                Exit Sub
